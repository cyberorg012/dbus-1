if "%APPVEYOR_REPO_TAG%" == "true" (
    ...
    if "%APPVEYOR_REPO_BRANCH%" == "barco_1.12.16" (
        conan create %APPVEYOR_BUILD_FOLDER% %APPVEYOR_PROJECT_NAME%/1.12.16.0@barco/healthcare -s build_type=%configuration% -s compiler="Visual Studio" -s compiler.version=%MSVC_VERSION%
        conan upload --remote barco --all --force %APPVEYOR_PROJECT_NAME%/1.12.16.0@barco/healthcare
    )
)
